...
func (d *Dialog) Handle(s *Session, text string) Reply {
	// Allow explicit restart via command or button text
	if text == "/start" || text == "Перезапустить" || s.State == StateStart {
		s.State = StateIntro
		greet := "🧡 Вас приветствует команда <b>концептуального жилого комплекса «ЗИМ Галерея»</b> — новаторского проекта бизнес-класса в Самаре с <i>уникальным домом галерейного типа и квартирами-ячейками</i>, вдохновлёнными легендарным Домом Наркомфина!"
	...
	case StatePayment:
		if text == PaymentCash || text == PaymentInstallment || text == PaymentMortgage || text == PaymentTradeIn {
			...
			msg := finalTextForSelection(s) + "\n\n" + "Оставьте номер телефона, мы пришлем вам каталог эксклюзивных предложений с ценами."
	...
	// When the bot doesn't understand, politely inform user and offer restart
	hint := "К сожалению, я не могу изменить уже выбранный вариант ответа. Вы можете пройти бота заново — отправьте /start."
	return Reply{Text: hint, Options: []string{""}, AdvanceTo: StateStart}
}
...
func finalTextForSelection(s *Session) string {
	...
	switch s.Payment {
	case PaymentCash:
		offer = "Те, кто использует <b>100% собственных средств</b> при оплате, могут получить <u>специальную премию от 2% до 3% от цены квартиры</u>."
	case PaymentInstallment:
		offer = "Бесплатная рассрочка от застройщика гибко подстраивается под ваши запросы, можно выбрать <b>размер первого взноса от 20%</b> и <u>удобную схему платежей – каждый месяц/квартал/полгода</u>."
	case PaymentMortgage:
		offer = "Для тех, кто использует ипотеку при оплате, существует <b>специальная премия до 3%</b> от цены квартиры. \n Кроме того, мы можем предложить вам траншевую ипотеку, которая <u>уменьшает ежемесячные платежи в 2 раза</u>, и вы не отказываетесь от привычных повседневных радостей."
	case PaymentTradeIn:
		offer = "Функция трейд-ин позволит вам <u>обменять старую квартиру на новую</u>: мы бесплатно реализуем ваш актив, высвободим ресурсы для нового выбора, при этом вы входите в сделку <b>с минимальным 5% первым взносом</b>."
	default:
...
// CatalogTitleFor был удалён за ненадобностью (выбор канала исключён)
