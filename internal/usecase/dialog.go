...
func finalTextForSelection(s *Session) string {
	...
	case PaymentMortgage:
		offer = "Для тех, кто использует ипотеку при оплате, существует <b>специальная премия до 3%</b> от цены квартиры. \n Кроме того, мы можем предложить вам траншевую ипотеку, которая <u>уменьшает ежемесячные платежи в 2 раза</u>, и вы не отказываетесь от привычных повседневных радостей. Хотите получить подробный расчет?"
	case PaymentTradeIn:
		offer = "Функция трейд-ин позволит вам <u>обменять старую квартиру на новую</u>: мы бесплатно реализуем ваш актив, высвободим ресурсы для нового выбора, при этом вы входите в сделку <b>с минимальным 5% первым взносом</b>. Хотите получить подробный расчет?"
	default:
