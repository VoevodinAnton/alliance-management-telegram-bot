...
func finalTextForSelection(s *Session) string {
	...
	case PaymentInstallment:
		offer = "Бесплатная рассрочка от девелопера гибко подстраивается под ваши запросы, можно выбрать размер первого взноса от 20% и удобную схему платежей – каждый месяц/квартал/полгода. Хотите получить подробный расчет?"
	case PaymentMortgage:
		offer = "Для тех, кто использует ипотеку при оплате, существует специальная премия до 3% от цены квартиры. \n Кроме того, мы можем предложить вам траншевую ипотеку, которая уменьшает ежемесячные платежи в 2 раза, и вы не отказываетесь от привычных повседневных радостей. Хотите получить подробный расчет?"
	case PaymentTradeIn:
		offer = "Функция трейд-ин позволит вам обменять старую квартируна новую: мы бесплатно реализуем ваш актив, высвободим ресурсы для нового выбора, при этом вы входите в сделку с минимальным 5% первым взносом. Хотите получить подробный расчет?"
	default:
